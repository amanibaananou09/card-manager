...
docker run -e "SPRING_PROFILES_ACTIVE=$PROFILE" -e "SERVER_PORT=$PORT" -d --network ess --name ${PROJECT_NAME} --restart=unless-stopped ${DOCKER_REGISTRY_URL}/${PROJECT_NAME}:${IMAGE_TAG}