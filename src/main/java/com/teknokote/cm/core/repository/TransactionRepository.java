...
public interface TransactionRepository extends JpaRepository<Transaction, Long> , CustomTransactionRepository
{
    ...
    @Query("SELECT new com.teknokote.cm.dto.TransactionChart(TO_CHAR(t.dateTime, 'DY'),t.product.name, t.card.cardId, SUM(t.quantity)) " +
    ...
    @Query("SELECT new com.teknokote.cm.dto.TransactionChart(TO_CHAR(t.dateTime, 'dd'),t.product.name, t.card.cardId, SUM(t.quantity)) " +
            "FROM Transaction t join t.card c where c.cardGroup.customer.id = :customerId " +
