...
public ResponseEntity<AccountDto> addAccount(@PathVariable Long customerId,@RequestBody AccountDto dto) {
    ...
    public ResponseEntity<AccountDto> updateAccount(@PathVariable Long customerId,@RequestBody AccountDto dto) {
        AccountDto savedAccount = accountService.update(dto);
        return new ResponseEntity<>(savedAccount, HttpStatus.CREATED);
    }
    @GetMapping(EndPoints.INFO)
    public ResponseEntity<AccountDto> getAccount(@PathVariable Long customerId,@PathVariable Long id)
    {
    ...
    public List<AccountDto> listAccountByActif(@PathVariable Long customerId,@PathVariable boolean actif) {
         return accountService.findAllByActif(actif);
    }


    @PostMapping(EndPoints.DEACTIVATE)
    public ResponseEntity<AccountDto> deactivateAccount(@PathVariable Long customerId,@PathVariable Long id) {
        return ResponseEntity.ok(accountService.deactivate(id));
    }

    @PostMapping(EndPoints.ACTIVATE)
    public ResponseEntity<AccountDto> activateAccount(@PathVariable Long customerId,@PathVariable Long id) {
